...
function renderTopContributors(repo: RepoData): string[] {
  ...
  return [
    ...
    ...contributors.map(
      (contributor) =>
        `| ${truncate(contributor.name, 32)} | ${contributor.commits} | ${contributor.percentage.toFixed(1)}% | ${formatReadableDate(contributor.firstCommit)} | ${formatReadableDate(contributor.lastCommit)} |`,
    ),
  ];
}
...
function renderMonthlyActivity(repo: RepoData): string[] {
  return [
    ...
    ...repo.monthlyActivity.map(
      (entry) =>
        `| ${format(parse(entry.month, "yyyy-MM", new Date()), "MMMM yyyy")} | ${entry.count} |`,
    ),
  ];
}
...
export async function exportMarkdown(
  ...
  const lines = [
    ...
    "",
    `> Generated by [repodash](${REPOSITORY_URL}) on ${formatReadableDate(generatedAt)}`,
    "",
    "## Overview",
    ...renderOverviewTable(data),
    ...(data.signals?.isShallowClone
      ? [
          "",
          "> Warning: this repository is a shallow clone, so commit and activity metrics may be incomplete.",
        ]
      : []),
    "",
  ...
  try {
    ...
    if (err.code === "EEXIST") {
      throw new Error(
        `Refusing to overwrite existing file: ${path.resolve(outputPath)}. Use --force to overwrite it.`,
      );
    }
